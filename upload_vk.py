...
def upload_to_vk(video_path, description="", title=""):
    ...
    try:
        ...
        # Prepare message
        message = description if description else "🏛️ Antik Kadınların Tarihi!\n\n#KadınTarihi #AntikTarih #Tarih #Eğitim"

        # Ensure message is not empty (VK requirement)
        if not message.strip():
            message = "🏛️ Yeni video!"
        ...
        video = upload.video(
            ...
            name=title or 'Antik Kadınların Tarihi',
...
def main():
    ...
    description = sys.argv[2] if len(sys.argv) > 2 else "Antik Kadınların Tarihi! 🏛️"
    title = sys.argv[3] if len(sys.argv) > 3 else "Antik Kadınların Tarihi"
